 ...
 * FilterBarComponent
 *
 * Filter selector for community content
 *
 * Features:
 * - 3 filter options (trending/latest/top)
...
@Component({
  ...
  template: `
    <div
      class="bg-white/5 backdrop-blur-sm border border-purple-500/20 rounded-xl p-4 mb-6"
    >
...
export class FilterBarComponent {
  ...
  getFilterClasses(filter: string): string {
    ...
    return `${baseClasses} ${
      filter === this.selectedFilter ? activeClasses : inactiveClasses
    }`;
  }
